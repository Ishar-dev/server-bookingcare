...
const getSpecialtiesByHospital = (req, res) => {
  ...
  try {
    ...
    const hospitalList = hospSpecs.filter(h => parseInt(h.hospital_id, 10) === id);

    if (hospitalList.length === 0) {
      return res.status(404).json({ message: "Không có bản ghi nào cho bệnh viện này." });
    }
    ...
    if (matchedSpecialties.length === 0) {
      return res.status(404).json({ message: "Không tìm thấy chuyên khoa nào." });
    }
